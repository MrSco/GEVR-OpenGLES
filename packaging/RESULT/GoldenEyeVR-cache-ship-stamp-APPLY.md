...
1. Each ship tag sets `GEVR_SHIP_TAG=vrNNN` in `gevr-vrNNN-boot.cmd` (public template does for vr441).
...
4. Optional compile-time default: `-DGEVR_SHIP_TAG_DEFAULT=\"vr441\"` when building `gevr_prepare.exe`.
