...
if (-not $SkipBuild) {
    ...
    try {
        & $buildScript -Target all -Vr -ImagesFileBacked 2>&1 | Tee-Object -FilePath (Join-Path $ProductRoot "build_vr440_filebacked.log")
...
Write-Host "[pack] Next: run _pack-vr440.ps1 -BuildDir $buildWindows"
